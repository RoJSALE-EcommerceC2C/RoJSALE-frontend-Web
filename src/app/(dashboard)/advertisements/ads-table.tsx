  ...
  switch (status) {
    ...
    case "Pending":
      return (
        <Badge
          variant="outline"
          className="bg-accent text-primary border-accent-foreground/10"
        >
          Pending
        </Badge>
      );
    ...
    case "Flagged":
      return (
        <Badge className="bg-primary text-primary-foreground">Flagged</Badge>
      );
  }
};
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2">
        ...
        {row.original.status === "Flagged" && row.original.flags && (
          <Badge className="bg-primary text-primary-foreground text-xs px-1.5 py-0.5 whitespace-nowrap">
            {row.original.flags} flags
          </Badge>
        )}
      </div>
    ),
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="border-b-gray-200">
                {headerGroup.headers.map((header) => (
                  <TableHead
                    key={header.id}
                    className="text-primary font-medium text-xs uppercase tracking-wider"
                  >
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} className="border-b-gray-200">
                  {row.getVisibleCells().map((cell) => (
                    <TableCell key={cell.id} className="py-3 text-primary">
                      {flexRender(
